...
\caption[caption]{estimated variances with\footnotemark Group 1: $\hat{\beta}$, Group 2: $\tilde{\beta}$}
\end{figure}

\footnotetext{Apparently running GAUSS in WINE on linux is not a good idea if one wants to use plotting. However, installing the linux light version (and writing several emails with aptech) does not result in a working environment either. There are rumours floating around about licenses given to students by the faculty which might solve my problem but what would really make me happy is using a proper programming language like e.g. R.}
